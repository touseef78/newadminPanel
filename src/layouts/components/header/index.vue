<template>
  <header>
    <b-row class="w-100 m-0">
      <b-col class="pl-18 pr-16 px-sm-24">
        <b-row align-v="center" align-h="between" class="w-100 position-relative">
          <b-col class="w-auto hp-flex-none hp-mobile-sidebar-button ml-4 ml-lg-0 mr-24 px-0">
            <b-button variant="text" class="btn-icon-only" @click="mobileSidebarShow = !mobileSidebarShow">
          ...
          <b-col v-if="searchActive" :class="`hp-header-search${searchClass ? ' active' : ''}`">
          ...
          <b-col class="hp-flex-none w-auto pr-0">
            <b-row align-v="center" align-h="end">
              <div class="w-auto px-0">
                <div class="d-flex align-items-center mr-4 hp-header-search-button">
                  <!-- Search Button -->
                  <b-button variant="text" class="btn-icon-only" @click="searchToggle()">
                    <i v-if="!searchActive" class="iconly-Curved-Search hp-text-color-black-60"></i>
                    <i v-else class="ri-close-line hp-text-color-black-60" style="font-size: 24px"></i>
                  </b-button>
    ...
    <b-sidebar id="mobile-sidebar" backdrop-variant="black-100" backdrop no-close-on-backdrop shadow bg-variant="white"
      class="hp-mobile-sidebar" body-class="p-0 d-flex flex-column" no-header-close no-header width="256px"
