                                    ...
                                    <div v-for="(mid, index) in mid_name" :key="index"
                                        ...
                                        <div class="d-flex align-items-center w-100">
                                            <b-form-input :id="'midpoint-' + index" type="text"
                                                ...
                                                @blur="geocodeAddress2(index)"></b-form-input>
                                            <ul class="dropdown-menu"
                                                v-if="suggestions2[index] && suggestions2[index].length > 0">
                                                <li v-for="(suggestion, suggestionIndex) in suggestions2[index]"
                                                    :key="suggestionIndex"
                                        ...
                                        <div v-if="mid.lat && mid.lng" class="mt-2">
                                            <b-badge variant="info">Lat: {{ mid.lat }}</b-badge>
                                            <b-badge variant="info" class="ml-2">Lng: {{ mid.lng }}</b-badge>
                                        </div>
                                    </div>
                                    <b-button variant="primary" @click="addMidpoint">Add New Field</b-button>
                                </b-form-group>
...
export default {
    data() {
        return {
            ...
            suggestions: [],
            // mid_name: [{ value: '' }],
            mid_name: [{ value: '', lat: null, lng: null }],
            suggestions2: [],
            highlightedIndex2: [-1],
            highlightedIndex: -1,
            // highlightedIndex2: -1,
    ...
    methods: {
        ...
        // geocodeAddress2() {
        //     var geocoder = new google.maps.Geocoder();
        //     var mid_name = this.address;
        //     var self = this; // Store reference to Vue component

        //     geocoder.geocode({ 'address': mid_name }, function (results, status) {
        //         if (status == 'OK') {
        //             self.mid_latitude = results[0].geometry.location.lat();
        //             self.mid_longitude = results[0].geometry.location.lng();
        //             console.log("Latitude: " + self.mid_latitude + ", Longitude: " + self.mid_longitude);
        //             // You can use drop_latitude and drop_longitude here as needed
        //         } else {
        //             console.error('Geocode was not successful for the following reason: ' + status);
        //         }
            ...
            if (address) {
                try {
                    ...
                    if (data.status === 'OK') {
                        ...
                        this.$set(this.mid_name[index], 'lat', location.lat);
                        this.$set(this.mid_name[index], 'lng', location.lng);
                    } else {
        ...
        addUser() {
            ...
            // Append each midpoint value separately
              ...
              ">
                ...
                ">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 col-12">
                                <b-form-group id="input-group-2" label="Midpoint Location:">
                                    <div v-for="(mid_name, index) in mid_name" :key="index"
                                        class="mb-2 d-flex align-items-center">
                                        <b-form-input :id="'midpoint-' + index" type="text"
                                            placeholder="Enter midpoint location" autocomplete="off"
                                            v-model="mid_name.value" @input="getLocationSuggestions2(index)"
