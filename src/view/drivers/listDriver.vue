    ...
    >
      ...
      >
        ...
        <path
            ...
            <b-button @click="updateStatus(row.item)" :variant="row.item.status === 'Approved' ? 'warning' : 'primary'" class="mb-8 mr-8">
...
export default {
  data() {
    return {
      ...
      perPage: 5, // Set a default number of rows per page
    ...
    fetchData() {
      axios
        ...
        .then((response) => {
          ...
          this.totalRows = this.users.length; // Update the totalRows based on the fetched data
        })
        .catch((error) => {
  ...
  editUser(userId) {
      // Use the $router.push method to navigate to the edit page
