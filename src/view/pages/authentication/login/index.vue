<template>
  <b-row class="hp-authentication-page">
    <left-content />

    <b-col cols="12" lg="6" class="py-sm-64 py-lg-0">
      <b-row align-v="center" align-h="center" class="h-100 mx-4 mx-sm-n32">
        <b-col
          cols="12"
          md="9"
          xl="7"
          class="col-xxxl-5 px-8 px-sm-0 pt-24 pb-48"
        >
          <h1 class="mb-0 mb-sm-24">Login</h1>
          <p class="mt-sm-8 mt-sm-0 text-black-60">
            Welcome back, please login to your account.
          </p>

          <b-form class="mt-16 mt-sm-32 mb-8">
            <b-form-group
              label="Email :"
              label-for="loginUsername"
              class="mb-16"
            >
              <b-form-input id="loginUsername" type="text"></b-form-input>
            </b-form-group>

            <b-form-group
              label="Password :"
              label-for="loginPassword"
              class="mb-16"
            >
              <b-form-input id="loginPassword" type="password"></b-form-input>
            </b-form-group>

            <b-row align-v="center" align-h="between" class="mb-16">
              <b-col class="hp-flex-none w-auto">
                <b-form-checkbox>Remember me</b-form-checkbox>
              </b-col>

              <!-- <b-col class="hp-flex-none w-auto">
                <b-link
                  class="hp-button text-black-80 hp-text-color-dark-40"
                  to="/pages/authentication/recover-password"
                >
                  Forgot Password?
                </b-link>
              </b-col> -->
            </b-row>

            <b-button to="/dashboard" type="submit" variant="primary" class="w-100">
              Sign in
            </b-button>
          </b-form>

          <!-- <div class="hp-form-info">
            <span class="text-black-80 hp-text-color-dark-40 hp-caption mr-4">
              Don’t you have an account?
            </span>

            <b-link
              class="text-primary-1 hp-text-color-dark-primary-2 hp-caption"
              to="/pages/authentication/register"
            >
              Create an account
            </b-link>
          </div>

          <div class="hp-or-line text-center mt-32">
            <span
              class="hp-caption text-black-80 hp-text-color-dark-30 px-16 bg-black-0 hp-bg-color-dark-100"
            >
              Or
            </span>
          </div>

          <div class="hp-account-buttons mt-32">
            <b-button variant="none" class="w-100 hp-border-color-dark-30 hp-hover-border-color-dark-primary-2">
              <svg
                width="14"
                height="14"
                viewBox="0 0 14 14"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                class="remix-icon"
              >
                <path
                  d="M3.28826 8.39085L2.82415 10.1235L1.12782 10.1593C0.620865 9.21906 0.333313 8.14325 0.333313 7.00002C0.333313 5.89453 0.602167 4.85202 1.07873 3.93408H1.07909L2.5893 4.21096L3.25086 5.7121C3.1124 6.11578 3.03693 6.54911 3.03693 7.00002C3.03698 7.4894 3.12563 7.95828 3.28826 8.39085Z"
                  fill="#FBBB00"
                ></path>
                <path
                  d="M13.5502 5.75455C13.6267 6.15783 13.6667 6.57431 13.6667 6.99996C13.6667 7.47726 13.6165 7.94283 13.5209 8.39192C13.1963 9.92012 12.3483 11.2545 11.1736 12.1989L11.1733 12.1985L9.27108 12.1014L9.00186 10.4208C9.78134 9.96371 10.3905 9.24832 10.7114 8.39192H7.14655V5.75455H10.7634H13.5502Z"
                  fill="#518EF8"
                ></path>
                <path
                  d="M11.1732 12.1986L11.1736 12.1989C10.0311 13.1172 8.57981 13.6667 6.99997 13.6667C4.46114 13.6667 2.25382 12.2476 1.12781 10.1594L3.28825 8.39087C3.85124 9.89342 5.3007 10.963 6.99997 10.963C7.73036 10.963 8.41463 10.7656 9.00179 10.4209L11.1732 12.1986Z"
                  fill="#28B446"
                ></path>
                <path
                  d="M11.2553 1.86812L9.09558 3.63624C8.4879 3.2564 7.76957 3.03697 6.99999 3.03697C5.26225 3.03697 3.78569 4.15565 3.2509 5.71208L1.0791 3.93406H1.07874C2.18827 1.79486 4.42342 0.333328 6.99999 0.333328C8.61756 0.333328 10.1007 0.909526 11.2553 1.86812Z"
                  fill="#F14336"
                ></path>
              </svg>
              <span>Continue with Google account</span>
            </b-button>

            <b-button variant="none" class="w-100 mt-16 hp-border-color-dark-30 hp-hover-border-color-dark-primary-2">
              <i class="ri-facebook-fill remix-icon text-primary"></i>
              <span>Continue with Facebook account</span>
            </b-button>
          </div>

          <div class="hp-other-links mt-24">
            <a href="javascript:;" class="text-black-80 hp-text-color-dark-40">
              Privacy Policy
            </a>

            <a href="javascript:;" class="text-black-80 hp-text-color-dark-40">
              Term of use
            </a>
          </div> -->
        </b-col>
      </b-row>
    </b-col>
  </b-row>
</template>

<script>
import {
  BRow,
  BCol,
  BButton,
  BLink,
  BForm,
  BFormGroup,
  BFormInput,
  BFormCheckbox,
} from "bootstrap-vue";
import LeftContent from "../LeftContent.vue";

export default {
  components: {
    BRow,
    BCol,
    BButton,
    BLink,
    BForm,
    BFormGroup,
    BFormInput,
    BFormCheckbox,
    LeftContent,
  },
};
</script>
